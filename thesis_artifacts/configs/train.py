...
def _tiered_budget_mb(free_mb: int, bs: int, reserve_mb: int) -> int:
    """Compute the per-bs VRAM budget.

    budget = (free_mb - reserve_mb) * (1 - tiered_margin(bs))

    The absolute reserve protects against the edge case where free_mb is tiny
    (e.g. another user holds most of the card) and a pure percentage margin
    collapses to a few hundred MB that can't cover allocator overhead.
    """
    usable = max(0, free_mb - reserve_mb)
...
def probe_batch_size(
    ...
    def budget_for(bs: int) -> int:
        return _tiered_budget_mb(free_mb, bs, reserve_mb)
