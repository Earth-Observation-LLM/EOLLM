...
(VLMs) a fixed catalogue of \textbf{12 question types} that probe urban
understanding and cross-view geolocalisation. The released corpus contains
\textbf{34{,}484 question records} grounded in \textbf{4{,}168 unique
...
\textbf{Cities}            & 40   & \textbf{Question records (release)} & 34{,}484 \\
\textbf{Countries}         & 32   & \textbf{Unique sample locations}    & 4{,}168 \\
\textbf{Question types}    & 12   & \textbf{Split strategies}           & 3 \\
...
\end{itemize}
In the released 34{,}484-record corpus ESRI covers \textbf{31{,}489}
questions (91\%), NAIP \textbf{1{,}946}, and IGN \textbf{1{,}049}; no
...
The 12 question types split into two families: \emph{urban
...
relationship between modalities or the camera geometry). Two
satellite-only tasks present in earlier drafts ---
\texttt{green\_space} and \texttt{road\_surface} --- were
\textbf{retired from the released benchmark}: both were solved almost
perfectly by every model we evaluated and therefore carried no
discriminative signal. All statistics in this report describe the
12-task release after their removal.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/fig14_task_taxonomy.pdf}
  \caption{The 12 question types organised by the ability they
...
The released corpus contains \textbf{34{,}484 question records}
...
\textbf{43{,}435 unique question identifiers}.
...
\begin{table}[H]
  ...
  \midrule
  per\_city          & 23{,}431 & 6{,}319 & --- \\
  seen\_unseen       & 23{,}282 & 8{,}179 & --- \\
  benchmark          & ---      & ---     & 4{,}734 (public) / 4{,}734 (w/ answers) \\
  \bottomrule
  \end{tabular}
...
\begin{figure}[H]
  ...
  \caption{Distribution of question records across the 12 topics.
  The ten "balanced" topics each contribute roughly 2{,}982 records;
  \texttt{junction\_type} (2{,}782) and
  \texttt{building\_height} (1{,}882) are smaller because they require
  OSM tags that are not present at every location.}
  \label{fig:topics}
\end{figure}
...
\begin{figure}[H]
  ...
  the seven urban-understanding topics; the remaining four modes
...
\subsection{Task structure and benchmark design}
The four panels below summarise the dataset from the angles most
relevant to a benchmark paper: which abilities the tasks probe, how
difficulty is distributed across them, how the geographic-generalisation
evaluation is constructed, and how the corpus is spread over world
regions.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig15_topic_by_family.pdf}
  \caption{Question records per task, coloured by the ability family
  they probe. Ten tasks are balanced at $\sim$2{,}982 records each;
  \texttt{junction\_type} and \texttt{building\_height} are smaller
  because their OSM tags are sparser. The seven urban-understanding
  tasks share a single overhead visual, while the five cross-view
  tasks drive the geolocalisation evaluation.}
  \label{fig:taskfamily}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig16_difficulty_by_topic.pdf}
  \caption{Difficulty composition per task. The \texttt{difficulty}
  field is a \emph{template-level} annotation assigned per task rather
  than estimated per question, so each task is internally homogeneous;
  the \texttt{hard} bucket is carried almost entirely by the
  \texttt{mismatch\_*\_hard} tasks with same-city distractors.}
  \label{fig:diffbytopic}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/fig17_generalisation_design.pdf}
  \caption{Geographic-generalisation evaluation design. The
  \texttt{seen\_unseen} strategy trains only on the 32 seen cities and
  validates exclusively on the 8 held-out (unseen) cities, while the
  disjoint benchmark mixes both city types ($3{,}807$ seen-city and
  $927$ unseen-city records). Together they let a model be scored on
  in-distribution and out-of-distribution geography under one taxonomy.}
  \label{fig:gendesign}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig18_region_coverage.pdf}
  \caption{Coverage by coarse world region. The corpus is
  Europe-heavy by construction but still spans six regions and both
  hemispheres; Russia and Türkiye are counted as Europe (their sampled
  cities sit on the European side). Unique-location counts (right) make
  the per-region sampling effort explicit.}
  \label{fig:regions}
\end{figure}
...
\paragraph{benchmark.}
A held-out 4{,}734-record evaluation set drawn from a disjoint mix
...
topic             & string  & One of the 12 topics. \\
...
\begin{itemize}[itemsep=3pt]
  \item \textbf{Correct-letter imbalance.} Within the 34{,}484-record
        release, answers A and B each occur $\sim$10{,}100 times while
        C and D each occur $\sim$7{,}100 times (Fig.~\ref{fig:diffans}).
  ...
  \item \textbf{Retired tasks: \texttt{green\_space},
        \texttt{road\_surface}.} Both were generated by the pipeline
        but \emph{removed from the released benchmark} because every
        evaluated model solved them almost perfectly, so they no longer
        separated systems. Re-adding them is a one-line change to
        \texttt{REMOVE\_QUESTION\_TYPES} in
        \texttt{splitting/split\_dataset.py}.
  \item \textbf{\texttt{water\_proximity} is defined but unreleased.}
        ...
        statistics in this report use the 12 topics actually present
        in the released JSONL.
\end{itemize}
...
small numbers of OSM-tag-dependent answers (e.g.\ a newly tagged
\texttt{building:levels} value flipping a \texttt{building\_height}
answer).
...
and Street View imagery across 40 cities and asks 12 task families
covering urban understanding and cross-view geolocalisation. The
release contains \textbf{34{,}484 question records} over \textbf{4{,}168
