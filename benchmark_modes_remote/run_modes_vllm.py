...
def main():
    ...
    ap.add_argument("--data-root", default=None,
                    help="root that image paths in the JSONL are relative to "
                         "(default: parent dir of --data, falling back to ./data)")
    ...
    # Image paths in the JSONL are relative to the data root. Default to the
    # benchmark file's parent dir so a remote --data path resolves images next to
    # it without a separate flag; --data-root overrides.
    global DATA_ROOT
    if args.data_root:
        DATA_ROOT = Path(args.data_root)
    else:
        DATA_ROOT = Path(args.data).resolve().parent
    print(f"[{args.model_name}] DATA_ROOT={DATA_ROOT}", flush=True)
